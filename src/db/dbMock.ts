...
function create(convidado: MockDb) {
  const id = String(Math.random() * 9999999);
  db.push({ id, ...convidado });
  return true;
}

function update(convidado: MockDb) {
  const index = db.findIndex((el) => el.id === convidado.id);
